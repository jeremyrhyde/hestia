<template>
  <div class="container mx-auto px-4 py-8">
    <div class="mb-8">
      ...
      <p class="mt-2 text-gray-600">Manage and control your smart home devices</p>
    </div>
    ...
    <div v-else-if="error" class="bg-red-50 border border-red-200 rounded-md p-4">
      <div class="flex">
        ...
        <div class="ml-3">
          <h3 class="text-sm font-medium text-red-800">Error loading devices</h3>
    ...
    <div v-else>
            ...
            <div class="flex items-center justify-between mb-4">
              <div class="flex items-center">
                ...
                <div class="ml-4">
                  ...
                  <p class="text-sm text-gray-500">{{ device.room }}</p>
                </div>
              </div>
              <div class="flex items-center">
                <span
      ...
      <div v-else class="text-center py-12">
        ...
        <p class="mt-1 text-sm text-gray-500">
          {{ searchQuery || selectedRoom || selectedType ? 'Try adjusting your filters' : 'No devices have been configured yet' }}
        </p>
      </div>
    </div>
  </div>
...
export default {
  ...
  setup() {
    ...
    const rooms = computed(() => {
      const roomSet = new Set(devices.value.map(device => device.room))
      return Array.from(roomSet).sort()
    })

    const deviceTypes = computed(() => {
      const typeSet = new Set(devices.value.map(device => device.type))
    ...
    const filteredDevices = computed(() => {
      return devices.value.filter(device => {
        const matchesSearch = !searchQuery.value || 
          ...
          device.room.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
          device.type.toLowerCase().includes(searchQuery.value.toLowerCase())
        
        const matchesRoom = !selectedRoom.value || device.room === selectedRoom.value
